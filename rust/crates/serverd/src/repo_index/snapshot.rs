...
pub(crate) fn build_repo_index_snapshot(
    ...
    for file in &identity.files {
        ...
        match mode {
            ChunkMode::WholeFile => {
                let file_bytes = fs::read(file.abs_path.as_path()).map_err(|e| {
                    RepoIndexError::with_detail("repo_index_walk_failed", e.to_string())
                })?;
            ...
            ChunkMode::FixedSize => {
                ...
                let mut handle = fs::File::open(file.abs_path.as_path()).map_err(|e| {
                    RepoIndexError::with_detail("repo_index_walk_failed", e.to_string())
                })?;
                ...
                loop {
                    let read = handle.read(buffer.as_mut_slice()).map_err(|e| {
                        RepoIndexError::with_detail("repo_index_walk_failed", e.to_string())
                    })?;
                    if read == 0 {
                        break;
...
fn push_chunk(
    ...
    let len = u64::try_from(chunk_bytes.len())
        .map_err(|_| RepoIndexError::new("repo_index_hash_failed"))?;
